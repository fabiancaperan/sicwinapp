        ...
        public List<CommerceModel> Build(List<SapModel> lstSap, List<EntidadesModel> entidades, List<CnbsModel> cnbs)
        {
            ...
            var lst = lstSap
                       .Join(entidades,
                        ...
                        //.Where(j => EF.Functions.Like(j.s.Cod_RTL, (j.e.fiid.PadRight(3)+"%") ) && j.s.Cod_RTL == j.e.fiid.PadRight(3))
                              ...
                              .Select(j => new CommerceModel
                              {
                                  ...
                                  Line = new StringBuilder().Append("02").Append(dat)
                                                            .Append(_format.Formato(j.FirstOrDefault()?.s.Nit.Trim(), 13, N)).Append(_format.Formato(RemoveSpecialCharacters(j.FirstOrDefault()?.s.NombreCadena.Trim()), 30, A))
